...
export default function HoursWorkflow() {
  ...
  return <div className="space-y-6">
    ...
    {weekId ? week.error ? <ErrorState message={hoursWorkflowError(week.error)} onRetry={() => void week.refetch()} /> : week.data ? <HoursWeekWorkspace key={week.data.id} week={toHoursWeekView(week.data)} readOnly={!canManage || !week.data.can_manage} onSaveDay={async input => { try { await week.mutation.mutateAsync({ type: 'save', ...input }); } catch (error) { throw new Error(hoursWorkflowError(error)); } }} onReview={async input => { try { await week.mutation.mutateAsync({ type: 'review', ...input }); } catch (error) { throw new Error(hoursWorkflowError(error)); } }} onClassify={async input => { await week.classificationMutation.mutateAsync(input); }} onReload={() => void week.refetch()} /> : <p role="status">Week laden…</p> : <>
