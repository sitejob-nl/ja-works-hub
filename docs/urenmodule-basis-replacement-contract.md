...
vijf stappen via de Supabase-MCP aangebracht (basismigratie, de twee indexen, de hercontrole van de
werkwijzeschakelaar uit reviewronde 1, de datumgetypeerde geldigheidsregel plus het stempelen van
`basis_version` uit reviewronde 2, en het delen van die geldigheidsregel met de resultaatvalidatie uit
reviewronde 3). Daarna is de bestandsversie `20260917090000` zelf in
...
`null` betekent dat de dag op dat moment geen enkele basis had. Een poging van vóór deze migratie die
wél een matrix noemde, heeft per definitie basis 0 vastgelegd (dat gebeurde in dezelfde transactie, en
vervangingen bestonden nog niet); de projectie leest die daarom als 0, zodat `null` één betekenis houdt.
...
één regel, als **datums** vergeleken, gedeeld door het optie-eindpunt, de vervangings-RPC én de
resultaatvalidatie in `hours_validate_classification_result`. Wat een vervanging mag vastleggen en wat
de finalisatie accepteert kunnen daardoor niet uit elkaar lopen. Een sessie met een andere `DateStyle`
kan de vergelijking niet omdraaien; de databaseproef draait beide RPC's onder `SQL, DMY`.
...
herberekening op de nieuwste schakel, en ook een geblokkeerde uitkomst die geen matrix noemde wordt
gemarkeerd. Een geopend formulier blijft bovendien wachten op een andere lopende dagactie, en een
optielijst die tegen een inmiddels vervangen basis is opgehaald wordt geweigerd — met de ingetypte
reden nog in beeld — in plaats van stilzwijgend gebruikt.

**Bewuste afwijking van de data-laag-conventie:** het ophalen van de matrixopties gebeurt op één klik en
staat buiten TanStack Query. Het is een eenmalige lees-actie waarvan het antwoord meteen wordt
beoordeeld; een cache zou hier vooral een verouderde lijst kunnen opleveren. De juistheid hangt niet aan
die keuze: de lijst draagt de basisversie waarvoor hij is opgehaald, en zodra die niet meer klopt
weigert het formulier te verzenden. Een portaalgebruiker ziet `matrix_basis: null` en een
...
| `22023` | Geen vastgelegde basis, ontbrekende of te lange reden, ontbrekende basisversie, niet-kiesbare matrixversie, dezelfde versie als de huidige basis, maximum bereikt, uitgeschakelde urenwerkwijze, of een al vrijgegeven dag |
| `PT409` | Dagversie of basisversie is **gewijzigd**; opnieuw ophalen en beoordelen. Een *ontbrekende* basisversie is invoer, geen conflict, en geeft `22023` — een herlaadlus zou die nooit oplossen |
