...
- **COM1:** `CommunicationTab.tsx` (op de opdrachtgever) aggregeert `communications` op `company_id` + contactpersonen, met realtime-subscriptie. Inkomende WhatsApp koppelt aan een bedrijfscontact via telefoon-lookup in `whatsapp-webhook` (alleen als het nummer géén kandidaat is; kandidaat houdt voorrang). **Nog niet:** auto-persistentie van álle inkomende e-mail (alleen handmatige triage via `EmailInbox.tsx` schrijft nu naar `communications`).
- **EM1:** `mail_accounts.reply_to_email` wordt via `buildReplyTo()` (`_shared/outlook-accounts.ts`) als Graph `replyTo` meegestuurd door `outlook-send.ts` + `outlook-send-mail` → antwoorden landen op het ingestelde adres (bv. info@). Instelbaar in de afzender-dialog (`OutlookSettings.tsx`, action `update_reply_to`). **Nog niet:** rol-routing van triage-taken (gaan nu altijd naar de huidige gebruiker, niet finance→administratie).
