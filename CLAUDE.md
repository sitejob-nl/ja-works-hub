...
> **Local / browser QA accounts live in `.env.local`** (not committed): `DEMO_ORG_*` is an **internal** admin login in the demo org `6dedabe4-…` (with seeded company/vehicle/property for E2E), `QA_SUPERADMIN_*` is a **superadmin-only** account (no org context — can't reach the main app UI). Use `DEMO_ORG_*` for Playwright QA of internal flows; enable the org **outbound kill-switch** (`organizations.settings.outbound_paused = {email,whatsapp:true}`) before exercising flows that send mail/WhatsApp, then revert. The local dev server runs against **production** Supabase (the committed `.env` points there), so `DEMO_ORG_*` logs in against real data. **Bij QA vanuit een git-worktree:** `.env`/`.env.local` zijn gitignored en staan dus NIET in een nieuwe worktree → kopieer ze er even in (`cp <repo-root>/.env .env && cp <repo-root>/.env.local .env.local`), anders crasht de dev-server op `supabaseUrl is required`. Ruim ze op na de QA.
...
- **`notes` is polymorf**: koppelt via `related_entity_type` (waarde `'kandidaat'`, `'bedrijf'`, …) + `related_entity_id` — er is **géén `candidate_id`** op `notes`. `recruiter_tasks` gebruikt hetzelfde `related_entity_type`/`related_entity_id`-patroon, en heeft naast `assigned_to` (toegewezene → `profiles`) ook `created_by` (→ `profiles`, nullable) om "door mij gemaakt" van "aan mij toegewezen" te onderscheiden; systeem-/AI-gegenereerde taken (cron, match-response, sick-report, signup, placement-triggers) hebben `created_by = NULL`.
...
- **Gewichten (genormaliseerd):** skills 50 / certifications 13 / functionGroup 12 / distance 20 (telt alléén mee als afstand bekend — anders geen straf) / availability 5. **Bonussen** (additief, nooit straf): taal +6, accommodatie +4, rijbewijs +5, **recency +5** (MG1 GAP2). Label: `rood` bij hardBlock of <45%, `groen` bij fit ≥72 + minstens één harde match, anders `oranje`.
...
### Transactionele e-mail — huisstijl-frame (BB1)

Eén gedeelde merk-wrapper voor álle transactionele mails: `supabase/functions/_shared/email-layout.ts` → `renderBrandedEmail({ theme, contentHtml, ... })` (witte kaart, logo, oranje accentbalk, navy koppen, grijze footer). **Multi-tenant:** `resolveBrandTheme()`/`loadBrandTheme()` lezen org-logo + `settings.accent_color` (HSL-triplet → hex via `hslTripletToHex`), default JA! Werkt (`#F97415`/`#0C4D78`). Senders bouwen alléén de body-inhoud en wrappen die; ze zetten `senderName: null` zodat de merk-footer de afzender-identiteit is (geen dubbele O365-handtekening). In gebruik door send-placement-confirmation (+ org-naam i.p.v. hardcoded "SiteJob"), -portal-invite, -timesheet-approval, -match-proposal, -ai-analysis en email-campaign-processor (wrapt alleen content-snippets — `looksLikeFullHtmlDoc`-guard). Client-side spiegel voor previews: `src/lib/email-brand-preview.ts` (gebruikt op de E-mailtemplates-pagina, mailflows klikbaar). **De frame is code, niet UI-bewerkbaar** — bij te sturen via logo + accentkleur in Instellingen; e-mailtemplate-teksten staan in `email_templates` (vrij) / `contract_templates` (juridisch).

### Communicatie-inbox + mailbox-routing (COM1 / EM1)

- **COM1:** `CommunicationTab.tsx` (op de opdrachtgever) aggregeert `communications` op `company_id` + contactpersonen, met realtime-subscriptie. Inkomende WhatsApp koppelt aan een bedrijfscontact via telefoon-lookup in `whatsapp-webhook` (alleen als het nummer géén kandidaat is; kandidaat houdt voorrang). Inkomende **e-mail** wordt ook automatisch vastgelegd: `outlook-mail` (`list`-actie) roept `_shared/inbound-mail-persist.ts` aan, dat **match-gated** persisteert — alleen mail van/over een bekende kandidaat of opdrachtgever(contact) (prioriteit kandidaat > contact > bedrijf), idempotent op `email_message_id`, in de achtergrond via `EdgeRuntime.waitUntil`. Onbekende afzenders worden NIET opgeslagen (AVG-minimaal + de CHECK `chk_comm_target` eist candidate_id of company_id). Handmatige triage (`EmailInbox.tsx`) blijft daarnaast bestaan (logt + maakt de taak).
- **EM1:** `mail_accounts.reply_to_email` wordt via `buildReplyTo()` (`_shared/outlook-accounts.ts`) als Graph `replyTo` meegestuurd door `outlook-send.ts` + `outlook-send-mail` → antwoorden landen op het ingestelde adres (bv. info@). Instelbaar in de afzender-dialog (`OutlookSettings.tsx`, action `update_reply_to`). **Rol-routing van triage-taken werkt:** per categorie (`CV` / `Klantvraag` / `Partner` / `Ruis` / `Review`) een vaste eigenaar in `organizations.settings.triage_routing` (in te stellen via `TriageRoutingSettings.tsx` in Instellingen); `EmailInbox` wijst de triage-taak toe aan `triage_routing[label]` met fallback naar de triërende gebruiker.
- **Mailboxrechten (03-09-2026):** `mail_account_user_access` is **leidend** (`_shared/outlook-accounts.ts` → `listVisibleAccounts` / `loadProviderForAccount` / `toAccountOption`). De admin-rol geeft géén impliciet lees-/verzendrecht op bedrijfsmailboxen; alleen de eigenaar van een *persoonlijke* koppeling heeft altijd toegang tot die mailbox. Admins beheren de rechten-matrix (Instellingen → Outlook) en moeten zichzelf óók een vinkje geven. Systeemmailers via `_shared/outlook-send.ts` gebruiken `bypassJaGrants` en zijn hier niet door geraakt.
- **Entiteitshistorie-filter:** `outlook-mail` action `list` met `participant_emails` bouwt **één** KQL-string voor Graph `$search` (`"participants:a OR participants:b"`, `_shared/outlook-mail-filter.ts`) én nafiltert **elke pagina** (ook `next_link`) op from/to/cc/bcc. Losse quoted termen aan elkaar met OR (`"participants:a" OR "participants:b"`) worden door Graph genegeerd en gaven de complete mailbox terug — dat toonde mail van derden op de opdrachtgever-tab. De client stuurt `participant_emails` daarom ook bij vervolgpagina's mee.
