...
export function readHoursFromMailText(text: string, context: MailReadingContext): WorkbookReading {
  ...
  // A holder rather than a plain binding: `record` fills this from inside a
  // closure, and a binding assigned only there still reads as null afterwards.
  const conflict = { found: null as { first: WorkbookCandidate; second: WorkbookCandidate } | null };
  ...
  for (const [index, raw] of lines.entries()) {
    ...
    let working = folded;
    const read = readDate(working, weekDates);
    if (read && !read.workDate) {
      note(row, lineText, 'De datum in deze regel valt niet in deze week.');
      continue;
    }
    // Past that guard a stated date is always a day of this week, which the
    // rest of this loop relies on; saying so once beats asserting it three times.
    const date: { workDate: string; text: string } | null =
      read && read.workDate ? { workDate: read.workDate, text: read.text } : null;
  ...
  function record(candidate: WorkbookCandidate, correction: boolean, superseded: string | null): void {
    ...
    if (!correction) {
      conflict.found ??= { first: existing.candidate, second: candidate };
      return;
    }
  ...
  const duplicate = conflict.found;
  if (duplicate) {
    return fail('DUPLICATE_DAY',
      `${duplicate.first.employeeName} staat meer dan één keer op ${duplicate.first.workDate}: `
      + `regel ${duplicate.first.row} en regel ${duplicate.second.row}. `
